<template>
            ...
            <tbody id="wallet-table-body" v-if="wallet">
              ...
              <tr>
                <td colspan="2">
                  ...
                  <button class="layui-btn layui-btn-primary layui-btn-xs" @click="openAccountDialog">
                    修改账户名
                  </button>
                  <input
                    type="button"
            ...
            <table class="layui-table table-center">
              <tbody>
                ...
                <tr>
                  <td><label for="">支付密码：</label></td>
                  <td>
                    <input type="password" class="inp" v-model="rechargeForm.payCode" />
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <table class="layui-table table-center">
              <tbody>
                <tr>
                  <td><label for="">钱包账户：</label></td>
                  <td>
                    <input type="text" class="inp" v-model="updataAccountForm.account" />
                  </td>
                </tr>
              </tbody>
            </table>
  ...
  const addWallet = () => {
    ...
    axios
      .post(
        `/api/store/walletRegister?account=${addForm.value.account}&password=${addForm.value.password}`,
        addForm.value,
        {
          headers: {
            Authorization: data,
  ...
  const updataAccount = () => {
    axios
      .put(
        `/api/wallet/updateAccount?id=${updataAccountForm.value.id}&account=${updataAccountForm.value.account}`,updataAccountForm,{
          headers: {
            Authorization: data 
          },
        }
  ...
  const recharge = () => {
    axios
      .put(
        `/api/wallet/recharge?mount=${rechargeForm.value.mount}&payCode=${rechargeForm.value.payCode}`,rechargeForm,{
          headers: {
            Authorization: data 
          },
        }
  ...
  const up = () => {
    axios
      .put(
        `/api/wallet/update?mount=${upForm.value.mount}&payCode=${upForm.value.payCode}`,upForm,
        {
          headers: {
            Authorization: data,
          },
  ...
  const updatePwd = () => {
    const pwdFormValue = {
      oldPwd: pwdForm.value.oldPassword,
      newPwd: pwdForm.value.newPassword,
      rePwd: pwdForm.value.rePassword,
    };
    ...
    axios
      ...
      .then((response) => {
        alert("更新成功");
        wallet.value.password = pwdForm.value.newPassword; // 更新本地的密码
        pwdDialogVisible.value = false;
      })
      .catch((error) => {
