    ...
    <div class="w">
      ...
      <div class="fr">
        <ul>
          ...
          <li></li>
          <li><a class="style_red" @click="brandInfo">我的信息</a></li>
        </ul>
      </div>
    </div>
  </section>
          ...
          <table class="layui-table table-center" style="margin-top: 15px">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <span>{{ brands.email }}</span>
                </td>
              </tr>
              <tr>
                <td><label for="">联系电话：</label></td>
                <td>
                  <span>{{ brands.phoneNumber }}</span>
                </td>
              </tr>
              <tr>
                <td><label for="">角色：</label></td>
                <td>
                  <span>{{ formatRole(brands.role) }}</span>
                </td>
              </tr>
            </tbody>
          </table>
...
axios
  ...
  .then((response) => {
    ...
    editForm.value = {
      id: brands.value.id,
      username: brands.value.username,
      password: brands.value.password,
      email: brands.value.email,
      phoneNumber: brands.value.phoneNumber,
      role: brands.value.role,
      avatar: brands.value.avatar // 确保 admins.value.avatar 是头像路径字符串
    };
    console.log(brands.value.avatar); // 确保输出头像路径或空字符串
  })
  .catch((error) => {
    console.error(error);
  });
...
const update = () => {
  axios
    .put("/api/brands/update", editForm.value, {
      headers: {
        Authorization: data,
      },
    })
    .then((response) => {
      ...
      brands.value = { ...editForm.value }; // 更新原始数据
      dialogVisible.value = false;
      console.log(brands.value.avatar);
    })
    .catch((error) => {
      console.error(error);
      alert("更新失败");
...
const updatePwd = () => {
  ...
  axios
    .patch("/api/brands/updatePwd", pwdFormValue, {
      headers: {
    ...
    .then((response) => {
      alert("更新成功");
      brands.value.password = pwdForm.value.newPassword; // 更新本地的密码
      pwdDialogVisible.value = false;
    })
    .catch((error) => {
